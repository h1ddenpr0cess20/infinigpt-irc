...
def _units_map(units: str) -> Dict[str, str]:
    ...
    Args:
        units: Unit system, e.g. ``"metric"`` (default), ``"imperial"``, or ``"us"``.
...
def _code_desc(code: int) -> str:
    """Translate an Open-Meteo weather code into a description.

    Args:
        code: The numeric weather code from the API.

    Returns:
        A human-readable English description for the code.
    """
    mapping = {
        0: "clear sky",
        1: "mainly clear",
...
def get_weather(city: str, units: str = "metric") -> Dict[str, Any]:
    ...
    Args:
        ...
        units: Unit system to use, ``"metric"`` (default) or ``"imperial"``/``"us"``.

    Returns:
        A mapping containing location details and current weather fields, or
        ``{"error": str}`` if the lookup fails.
    """
